...
reproducir_audio_aleatorio = function(lista_audios, prioridad_reproduccion = PRIORIDAD_SONIDOS, reproducir_en_bucle = false, volumen = 1, tonalidad = 1) {
	...
	// Reproducir el audio
	self.detener_audio(audio_seleccionado);
...
reproducir_cancion = function(nombre_audio) {
	// Reproducir cancion
	return self.reproducir_audio(nombre_audio, PRIORIDAD_CANCIONES);
}
...
reproducir_cancion_bucle = function(nombre_audio, inicio_bucle = 0, final_bucle = noone) {
	...
	if ((inicio_bucle == 0) and (final_bucle == noone)) {
		return self.reproducir_audio(nombre_audio, PRIORIDAD_CANCIONES, true);
	} else if (final_bucle == noone) {
		final_bucle = audio_sound_length(nombre_audio);
	}
	
	// Declarar variables
	var cancion = self.reproducir_audio(nombre_audio, PRIORIDAD_CANCIONES, true);
