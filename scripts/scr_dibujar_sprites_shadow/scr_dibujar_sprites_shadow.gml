function dibujar_sprites_shadow() {
    // Estando de pie
    ...
    if (((velocidad_horizontal != 0) and tocando_suelo) and (accion == 0)) {
        ...
        sprite_actual = spr_shadow_saltando;
        fotograma += 0.25;
    ...
    // Frenar en el suelo
    if (accion == -4) {
        sprite_actual = spr_shadow_frenando;
    ...
    if ((accion == 1) and (velocidad_vertical > 0) and (collision_line(x, y, x, y + 80, obj_superficie, true, true) or velocidad_vertical > 9)) {
        if (sprite_actual != spr_shadow_aterrizar) {
            indice_sprite = 0;
        }
        sprite_actual = spr_shadow_aterrizar;
    ...
    if (accion == 4.5) {
        fotograma += 0.5;
        sprite_actual = spr_shadow_impulso_aereo;
    }

    // Saltar en un trampolin
    if (accion == 5) {
        if (sprite_actual == spr_shadow_saltando) {
            if (indice_sprite >= 3) {
                indice_sprite = 0;
            }
            fotograma += 0.25;
        ...
        if (sprite_actual == spr_shadow_impulsarse_pared) {
            fotograma += 0.25;
        ...
        if (sprite_actual == spr_shadow_cinta_moebius) {
            if (indice_sprite <= 10) {
                fotograma += 0.25;
            } else {
                accion = 0;
    ...
    // Walljump
    if (accion == 6) {
        sprite_actual = spr_shadow_impulsarse_pared;
        fotograma += 0.25;
    }
    ...
    // Deslizarse
    if (accion == 9) {
        sprite_actual = spr_shadow_deslizarse_piso;
        fotograma += 0.5;
    }
    ...
    if ((accion == 11) and colision_con_riel() and tocando_suelo) {
        sprite_actual = spr_shadow_deslizarse_riel;
        fotograma += 0.25;
    }

    // Usar swingbar
    if (accion == 12) {
        sprite_actual = spr_shadow_usando_swingbar;
        fotograma += 0.35;
    }

    if (accion == 13) {
        sprite_actual = spr_shadow_rodando_b;
        fotograma += 1;
    }
    ...
    if (accion == 15) {
        sprite_actual = spr_shadow_superar_qte;
        fotograma += 0.2545;
    ...
    if (accion == 15.5) {
        sprite_actual = spr_shadow_fallar_qte;
    ...
    if (accion == 16) {
        sprite_actual = spr_shadow_cinta_moebius;
    }

    if (accion == 16.5) {
        if (direccion_horizontal == 1) {
            sprite_actual = spr_shadow_cinta_moebius_riel_a;
        }
        else {
            sprite_actual = spr_shadow_cinta_moebius_riel_b;
        }
    }
    ...
    // Pisoton
    if (accion == 18) {
        sprite_actual = spr_shadow_pisoton;
        fotograma += 0.5;
    }

    if (accion == 18.5) {
        sprite_actual = spr_shadow_iniciar_pisoton;
        fotograma += 0.5;
    }

    // Walljump
    if (accion == 19) {
        sprite_actual = spr_shadow_sostenerse_pared;
    }
    ...
    if (accion == 25) {
        sprite_actual = spr_shadow_jump_panel;
    }
    ...
    if (accion == 29) {
        sprite_actual = spr_shadow_poste_giratorio;
        fotograma += 0.25;
    }

    // Salto de bungee
    if (accion == 30) {
        sprite_actual = spr_shadow_salto_bungee;
        fotograma += 0.25;
    }
    ...
    if (accion == 35.2) {
        sprite_actual = spr_shadow_rotate_jump_launcher;
        fotograma += 0.5;
    ...
    // Correr por la pared
    if (accion == 36) {
        sprite_actual = spr_shadow_correr_pared;
        fotograma += 0.5;
    }
